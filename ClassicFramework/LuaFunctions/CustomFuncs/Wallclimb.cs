    ...
    internal static class Wallclimb
    {
        ...
        private static int FuncBody(IntPtr parLuaState)
        {
            ...
            if (val >= 0 && val <= 0.64)
            {
                ...
                Console.WriteLine("Custom: " + AntiWarden.HookWardenMemScan.GetHack("Wallclimb").CustomBytesToString);
                Console.WriteLine("Normal: " + AntiWarden.HookWardenMemScan.GetHack("Wallclimb").OriginalBytesToString + " - as float: " + 
                    BitConverter.ToSingle(AntiWarden.HookWardenMemScan.GetHack("Wallclimb").OriginalBytes, 0));
